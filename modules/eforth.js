...
export function Forth(output=console.log) {
    ...
    const _init = ()=>{                                 ///< dictionary constructor
        ...
        let vm_last = ()   =>{ return vm.dict.tail(2).pf.tail() }          ///< pf of last word created
        ...
        new Prim("create",   "cm", c=>vm.dict.add()),                      // create new word
    ...
    let exec = (cmd)=>{                                     ///< outer interpreter
        if (vm.dict.length==0) _init()                      /// * construct dict now
