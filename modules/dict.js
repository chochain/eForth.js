...
export const init = (vm)=>{                              ///< dictionary constructor
    ...
    [
        ...
        new Immd("else",  "br", c=>br._else(vm)),
        new Immd("then",  "br", c=>br._then(vm)),
        /// @}
        /// @defgroup Loop ops
        /// @brief begin.{pf}.again, begin.{pf}.until, begin.{pf}.while.{pf1}.repeat
        /// @{
        new Immd("begin", "br", c=>br._begin(vm)),
        new Immd("while", "br", c=>br._while(vm)),
        new Immd("repeat","br", c=>br._repeat(vm)),
        new Immd("again", "br", c=>br._again(vm)),
        new Immd("until", "br", c=>br._until(vm)),
        /// @}
        /// @defgroup Loop ops
        /// @brief for.{pf}.next, for.{pf}.aft.{pf1}.then.{pf2}.next
        /// @{
        new Immd("for",   "br", c=>br._for(vm)),
        new Immd("aft",   "br", c=>br._aft(vm)),
        new Immd("next",  "br", c=>br._next(vm)),
        ...
        /// @{
        new Prim("?",     "mm", c=>vm.log(mm.at(vm.dict[pop()]))),
        new Prim("@",     "mm", c=>push(mm.at(vm.dict[pop()]))),                           // w -- n
        new Prim("!",     "mm", c=>{ let w=pop(); mm.store(vm.dict[w], pop()) }),          // n w  --
        new Prim("+!",    "mm", c=>{ let w=pop(); mm.storeplus(vm.dict[w], pop()) }),      // n w --
        new Prim("allot", "mm", c=>{                                                       // n --
            vm.nvar(_dovar, 0)                                                             // create qf array
            for (let n=pop(), i=1; i<n; i++) mm.arr_store(vm.tail(), i, 0)
        }),
        new Prim("n?",    "mm", c=>{                                       // w i --
			let i=pop()
			vm.log(mm.arr_at(vm.dict[pop()], i))
		}),
        new Prim("n@",    "mm", c=>{                                       // w i -- n
			let i=pop(), w = pop()
			push(mm.arr_at(vm.dict[w], i))
		}),
        new Prim("n!",    "mm", c=>{                                       // n w i --
			let i=pop(), w = pop()
			mm.arr_store(vm.dict[w], i, pop())
		}),
        /// @}
        ...
        new Prim("date",     "os", c=>vm.log(os.date()+" ")),
        new Prim("time",     "os", c=>vm.log(os.time()+" ")),
